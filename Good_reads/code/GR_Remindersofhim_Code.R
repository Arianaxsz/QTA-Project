...
url <- "https://www.goodreads.com/book/show/58346977/reviews?reviewFilters={%22workId%22:%22kca://work/amzn1.gr.work.v1.ZrY3ahHDHAcBG37oVaZvdQ%22,%22after%22:%22MzcxMTIsMTY0MjIwNDU5MzAzOA%22}"

book.title <- "Reminders of Him"
output.filename <- "GR_RemindersofHim.csv"
...
for(t in 1:98){
  ...
  reviews <- remDr$findElements("css selector", ".ReviewsList , .ReviewsList__listContext , .Text__subdued , .ReviewCard__content")
  reviews.html <- lapply(reviews, function(x){x$getElementAttribute("outerHTML")[[1]]})
  reviews.list <- lapply(reviews.html, function(x){read_html(x) %>% xml2::xml_text()} )
  ...
  NextPageButton <- remDr$findElement("css selector", "#__next , .PageFrame--siteHeaderBanner , .BookReviewsPage , .BookReviewsPage__gridContainer , .BookReviewsPage__rightColumn , .ReviewsList , .Divider--largeMargin span") 
